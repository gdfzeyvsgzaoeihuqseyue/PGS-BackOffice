<template>
  <div class="min-h-screen flex items-center justify-center bg-slate-50 relative overflow-hidden">
    ...
    <div class="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div class="absolute -top-[10%] -left-[10%] w-[50%] h-[50%] bg-emerald-400 rounded-full mix-blend-multiply filter blur-[128px] opacity-20 animate-pulse"></div>
        <div class="absolute top-[20%] -right-[10%] w-[40%] h-[40%] bg-cyan-400 rounded-full mix-blend-multiply filter blur-[128px] opacity-20 animate-pulse delay-1000"></div>
        <div class="absolute -bottom-[10%] left-[20%] w-[40%] h-[40%] bg-purple-400 rounded-full mix-blend-multiply filter blur-[128px] opacity-20 animate-pulse delay-2000"></div>
    </div>

    <div class="relative w-full max-w-md bg-white/70 backdrop-blur-xl border border-white/50 shadow-2xl shadow-emerald-500/10 rounded-3xl p-8 md:p-10 transform hover:scale-[1.01] transition-transform duration-500">
      <div class="text-center mb-10">
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-400 to-cyan-500 shadow-lg shadow-emerald-500/30 mb-6 text-white">
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        <div v-if="error" class="p-4 bg-red-50 text-red-600 text-sm font-medium rounded-xl border border-red-100 flex items-start gap-3 animate-fade-in-up">
...
const handleLogin = async () => {
    ...
    try {
        ...
        // Navigation handled in store or global middleware
    } catch (e) {
