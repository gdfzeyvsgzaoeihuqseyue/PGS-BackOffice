               ...
               <tr v-for="user in users" :key="user.id" class="hover:bg-slate-50/50 transition-colors group">
                  <td class="px-6 py-4">
                     <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-100 to-cyan-100 text-blue-600 flex items-center justify-center font-bold text-sm">
...
// Correct endpoint as per user request: /api/v1/superadmin/leaner/list
const { data, refresh, error } = await useAPI('/superadmin/leaner/list', {
    query: computed(() => ({
        page: page.value,
        limit,
...
const users = computed(() => {
    ...
    return data.value.items || data.value.learners || data.value
})
...
const toggleStatus = async (user) => {
    try {
        const action = user.isActive ? 'suspend' : 'activate'
         // Correct endpoint: /api/v1/superadmin/leaner/manage
